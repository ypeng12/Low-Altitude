...
### 3. 步骤 2 深度复盘：人工审定边界规则与剔除逻辑

#### A. 初始状态与问题提出
在人工审定前，从原始游客评论中抽取的候选词库共计包含 8,726 个独特词条，其中混杂了大批非情感词、实体名称、社交客套话、程序化服务评价与物理飞行震动词。单纯依靠词频或正则筛选会导致严重的虚假正例（False Positives）。

#### B. 核心学术研讨与推导过程（讨论、思考与筛选逻辑）
1. **体验者心理状态 ($E_1$) 与 刺激物/服务评价 ($E_2$)**：
   - 咱们深入讨论了是否仅保留游客直接感受到的内部情绪（$E_1$: *nervous, afraid, scared, thrilled, tranquil*），还是同时保留对低空航线属性的主观评价（$E_2$: *scary, spectacular, smooth, professional, flawless*）。
   - *最终裁决*：遵照旅游与酒店管理顶级 NLP 框架，将 $E_1$ 与 $E_2$ 共同保留为自建金标准代码本的核心维度。
2. **剔除边界规则 1：社交客套词与感叹词**：
   - 针对 *thanks, thank, thankyou* 以及 *wow, yay* 等词汇进行了讨论。
   - *最终裁决*：判定为社交礼貌问候与非正式感叹，予以全量剔除，不纳入正式情感词库。
3. **剔除边界规则 2：地理与物理实体名称**：
   - 原始高频词中的 *grand*（大峡谷 Grand Canyon）、*maui*（茂宜岛）、*helicopter*（直升机）、*glacier*（冰川）、*canyon*（峡谷）等词汇频次极高。
   - *最终裁决*：将所有 8,096 个物理实体、地理名词与品牌词全量剔除。
4. **剔除边界规则 3：经济价格与成本评价**：
   - 针对 *expensive, overpriced, cheap, price, cost* 等词汇进行辨析。
   - *最终裁决*：判定为经济成本评价而非纯粹心理情绪，将其剥离并转化为计量回归模型中的独立控制变量 (`price_value_mention`)。
5. **剔除边界规则 4：程序化服务效率与认知立场**：
   - 针对 *informative, timely, seamless, knowledgeable* 等词汇。
   - *最终裁决*：判定为客观服务效率评价，予以剔除。
6. **剔除边界规则 5：物理颠簸与运动感知**：
   - 针对 *choppy*（气流颠簸）等词汇进行辨析。
   - *最终裁决*：判定为客舱物理运动触感而非内部心理情感，予以剔除。

#### C. 人工审定与校准总结
- **原始候选宇宙**：全量筛选 8,726 个独特词条。
- **留存主金标准代码本**：**630 个纯情感与评价词** (`gold_emotion_lexicon_codebook.xlsx`)。
- **主排除非情感日志**：**8,096 个排除词** (`removed_non_emotion_words_log.xlsx`)。
- **零重叠校验**：$630 	ext{ 金标准代码本} \cap 8,096 	ext{ 主排除日志} = 0$（100% 零重叠保证划分）。
...
│
└── 2️⃣ 完全被 NRC 词典遗漏的 3 大类别 (领域缺口) ────── 272 个词 (43.17%) ⭐ (论文核心亮点)
    ├── 2a. 第 1 类：语法分词与形态衍生词缺口 ─────────── 127 个词 (占比 46.69% | 语料频次 17,565 次)
    │       (如: loved, impressed, inspiring, relaxed, scared, amazed, thrilled, better, safer)
    ├── 2b. 第 2 类：网络游客高频口语赞誉词 ───────────── 128 个词 (占比 47.06% | 语料频次 28,401 次)
    │       (如: great, awesome, fantastic, nice, incredible, comfortable, fabulous, enjoyable)
    └── 2c. 第 3 类：低空观光旅游垂直领域特有词汇 ─────── 17 个词 (占比 6.25% | 语料频次 2,862 次) ⭐
            ├── 子维度 A: 空中高空美学视觉震撼词 (breathtaking, stunning, scenic, awe, surreal, sublime)
            └── 子维度 B: 低空飞行感知风险与躯体症状词 (airsick, claustrophobic, jitters, phobia)
```
...
| 领域专有情绪子维度 (`Domain Category`) | 严谨中文概念与理论归因 | 典型代表词汇与语料频次 ($N=17$ 个词) |
| :--- | :--- | :--- |
| **Awe** | **敬畏 / 震撼** *(高空俯瞰视觉高唤起与美学崇高感)* | *breathtaking (1,346), stunning (552), scenic (400), awe (304), surreal (98), breathtakingly (30), mesmerizing (26), awed (15), stunningly (10), sublime (6), spellbinding (4)* |
| **Embodied Distress** | **身体化不适 / 身体性负面体验** *(飞行晕机与躯体生理症状)* | *airsick (33)* |
| **Situational Anxiety** | **情境性焦虑** *(小型机舱密闭空间压抑、气流颠簸与高空悬浮心理)* | *claustrophobic (16), claustrophobia (9), jitters (5), unnerving (4), phobia (4)* |

##### 🌟 第 3 类低空特有词汇全量明细表 ($N=17$ 个单词变体 / 3 大精细子维度划分)

###### 子维度 3A：Awe (敬畏 / 震撼 / 壮美感) [11 个词, 累计 2,791 次提及]
| 评论原始单词 (`word`) | 规范字典词根 (`canonical_lemma`) | 21,215 语料频次 | 严谨中文翻译 | 理论与实证归因结论 |
| :--- | :--- | :---: | :--- | :--- |
| **`breathtaking`** | **`breathtaking`** | **1,346** | 令人屏息震撼的 | 冰川/大峡谷高空俯瞰的主导美学震撼描述词。 |
| **`stunning`** | **`stunning`** | **552** | 令人目眩震撼的 | 高空视角带来的视觉高唤起冲击。 |
| **`scenic`** | **`scenic`** | **400** | 风景优美的 / 宜人的 | 景观美学欣赏表达。 |
| **`awe`** | **`awe`** | **304** | 敬畏 / 震撼 | 核心美学情绪 (Awe / Keltner & Haidt 2003)。 |
| **`surreal`** | **`surreal`** | **98** | 超现实的 / 梦幻般的 | 俯瞰自然奇观时的梦幻悬浮体验。 |
| **`breathtakingly`**| **`breathtakingly`**| **30** | 令人屏息震撼地 | 美学惊叹程度副词。 |
| **`mesmerizing`** | **`mesmerizing`** | **26** | 令人目眩神迷为之倾倒的 | 沉浸于空中景致的催眠般美学体验。 |
| **`awed`** | **`awed`** | **15** | 感到肃然起敬震撼的 | 体验者感到震撼的被动心理状态。 |
| **`stunningly`** | **`stunningly`** | **10** | 令人震撼地 | 视觉冲击程度副词。 |
| **`sublime`** | **`sublime`** | **6** | 崇高壮丽绝美的 | 高空视角引发的康德式美学崇高感 (Sublime)。 |
| **`spellbinding`** | **`spellbinding`** | **4** | 扣人心弦迷人的 | 观光过程中的高度注意力吸引。 |

###### 子维度 3B：Embodied Distress (身体化不适 / 身体负性体验) [1 个词变体, 33 次提及]
| 评论原始单词 (`word`) | 规范字典词根 (`canonical_lemma`) | 21,215 语料频次 | 严谨中文翻译 | 理论与实证归因结论 |
| :--- | :--- | :---: | :--- | :--- |
| **`airsick`** | **`airsick`** | **33** | 晕机的 *(生理躯体反应)* | 小型观光飞机/直升机的生理晕机与恶心症状。 |

###### 子维度 3C：Flight Anxiety (飞行焦虑 / 空中不安) [5 个词变体, 38 次提及]
| 评论原始单词 (`word`) | 规范字典词根 (`canonical_lemma`) | 21,215 语料频次 | 严谨中文翻译 | 理论与实证归因结论 |
| :--- | :--- | :---: | :--- | :--- |
| **`claustrophobic`**| **`claustrophobic`**| **16** | 感到幽闭恐惧压抑的 | 小型机舱密闭空间带来的空间压抑感。 |
| **`claustrophobia`**| **`claustrophobia`**| **9** | 幽闭恐惧症 | 密闭飞行环境诱发的恐慌与幽闭心理。 |
| **`jitters`** | **`jitters`** | **5** | 忐忑不安 / 紧张发抖感 | 登机前与飞行中的躯体抖动与紧张感。 |
| **`unnerving`** | **`unnerving`** | **4** | 令人发慌不安吓人的 | 气流颠簸引发的心理失控发慌感。 |
| **`phobia`** | **`phobia`** | **4** | 恐惧症 / 恐高心理 | 高空悬浮诱发的恐高心理反应。 |
