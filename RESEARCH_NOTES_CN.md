...
│               608 个纯正情感词 | 错别字归一化 (canonical_lemma) | 8,094 个剔除词 (零重叠)                │
└─────────────────────────────────────────────────────────────────────────────────────────────────────────┘
```
...
### 5. 数学完备性证明与全量文件指南
$$\text{全量语料库核心词汇池 (8,726 个词)} = \text{Master 金标准代码本 (632 个词)} + \text{Master 剔除词日志 (8,094 个词)}$$
$$\text{Master 金标准代码本 (608)} \cap \text{Master 剔除词日志 (8,094)} = 0 \quad (\text{100% 零交集完备划分})$$
...
| **Master 被剔除词日志** | **Excel / CSV** | **8,094 个词** | **核心主审计日志**，包含所有被剔除的中性词、实体词、人名地名与时间词。 | 👉 [`removed_non_emotion_words_log.xlsx`](file:///Users/yuliangpeng/Desktop/Low-Altitude/data/derived_outputs/removed_non_emotion_words_log.xlsx) |
