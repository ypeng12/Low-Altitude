...
# Filter English reviews (N=21,238) to avoid non-English false zeros
...
def get_official_nrc_category(word):
    ...
    if not emotions:
        return 'Trust'  # Default positive/neutral emotion fallback
    
    # Priority hierarchy for multi-emotion words: Negative emotions > High arousal > Joy/Trust
...
# -------------------------------------------------------------
# Generate Publication-Grade Scatter Plot for Tourism Management
# -------------------------------------------------------------
...
# Theoretical Quadrant Annotation Boxes
ax.text(-0.70, 4.97, 'Quadrant II: Risk Perception & Reassurance\n(High Rating, Perceived Risk Mention)', 
        fontsize=9.5, fontweight='bold', color='#334155', alpha=0.6, va='top')
ax.text(0.72, 4.97, 'Quadrant I: Core Satisfaction & Delight\n(High Rating, High Sentiment Polarity)', 
        fontsize=9.5, fontweight='bold', color='#15803D', alpha=0.7, va='top')
ax.text(-0.70, 2.40, 'Quadrant III: Severe Service Failure\n(Low Rating, Dissatisfaction)', 
        fontsize=9.5, fontweight='bold', color='#B91C1C', alpha=0.65, va='bottom')
ax.text(0.72, 2.40, 'Quadrant IV: Friction & Value Deficit\n(Low Rating, Operational Complaints)', 
        fontsize=9.5, fontweight='bold', color='#C2410C', alpha=0.65, va='bottom')
...
# Clean Legend for Official 8 NRC Emotions + CATE Feature
ax.legend(
...
plt.tight_layout()
output_path = 'figures/word_sentiment_scatter_academic.png'
plt.savefig(output_path, dpi=300, bbox_inches='tight')
plt.close()

print(f"Publication-ready 8-NRC emotion scatter plot saved to {output_path}")
